...
filenames = os.listdir(r'DATA\UKData_by_hour\week9\LowFreq')
# print(filenames)
...
for time in range(7 * 24):
    ...
    for i in filenames:
        if (i[:10] == str(1456704000 + 3600 * time)):#换周的话记得换一下时间戳喔
            name = i[11:-4]
            name_data = np.load('DATA/UKData_by_hour/week9/LowFreq/' + i)
...
dp = package_dataprocess.Nilm_classes.Nilm_dataprocess(path_csv="DATA/UKData_by_hour/week9/vicurve_name.xlsx",
                                                       path_img="DATA/UKData_by_hour/week9/vicurve32x32/")
...
val_steps = (20000- dp.lookback) // dp.batch_size
...
model.save("my_h5_model_CONCA_continue.h5")
'''
